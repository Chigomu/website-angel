...
<head>
    ...
    <style>
        ...
        .detail-card {
            ...
            border: 1px solid var(--line-color); overflow: hidden; display: grid; grid-template-columns: 400px 1fr; min-height: 500px;
        }
        .product-image-col {
        ...
        .info-section h4 { font-family: var(--font-body); font-size: 0.9rem; text-transform: uppercase; color: var(--text-light); margin-bottom: 8px; letter-spacing: 1px; }
        .info-section p { color: var(--text-dark); font-size: 1rem; line-height: 1.7; }
        .tech-data {
            margin-top: auto; background: #fdfdfd; padding: 20px; border-radius: 8px; border: 1px solid #eee;
            font-size: 0.85rem; display: grid; grid-template-columns: 1fr 1fr; gap: 15px;
        }
        ...
        .btn-back { padding: 12px 25px; border: 1px solid var(--line-color); color: var(--text-dark); text-decoration: none; font-weight: 500; transition: 0.3s; }
        ...
        <div class="detail-card reveal active">
            ...
            <div class="product-info-col">
                ...
                <div class="tech-data">
                    ...
                    <div class="tech-item"><span>Tanggal Dibuat</span><strong><?= date('d M Y, H:i', strtotime($product['created_at'])) ?></strong></div>
                    <div class="tech-item"><span>Terakhir Update</span><strong><?= date('d M Y, H:i', strtotime($product['updated_at'])) ?></strong></div>
                </div>
                <div class="action-bar">
                    <a href="edit_product.php?id=<?= $product['id'] ?>" class="btn-primary"><i class="fas fa-edit"></i> Edit Produk</a>
                    <a href="delete_product.php?id=<?= $product['id'] ?>" class="btn-back" onclick="return confirm('Hapus produk ini?')" style="border-color: #c0392b; color: #c0392b;"><i class="fas fa-trash"></i> Hapus</a>
                </div>
            </div>
        </div>
    </div>
